...
export async function clientPagination(
  accessToken: string,
  refreshToken: string,
  skip: number,
  limit: number
  ...
  // try {
  const { data, status } = await instance.get(
    `/clients/pagination?limit=${limit}&skip=${skip}`,
    {
      headers: { Authorization: `Bearer ${accessToken} ${refreshToken}` },
    }
  );
...
export async function contactUsPagination(
  accessToken: string,
  refreshToken: string,
  skip: number,
  limit: number
  ...
  // try {
  const { data, status } = await instance.get(
    `/contacts-details/pagination?limit=${limit}&skip=${skip}`,
    {
      headers: { Authorization: `Bearer ${accessToken} ${refreshToken}` },
    }
  );
